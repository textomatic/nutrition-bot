...
def main():
    ...
    if submit and query is not None:
        ...
        with respp: # One column for answers from research paper
            ...
            if len(ans_respp) == 0: # It is possible that results may be empty due to low confidence scores
                st.markdown(" ")
        ...
        with socmed: # One column for answers from social media (reddit)
            ...
            if len(ans_socmed) == 0: # It is possible that results may be empty due to low confidence scores
                st.markdown(" ")
                st.markdown("#### No results!")
                st.markdown("---")
            else:
